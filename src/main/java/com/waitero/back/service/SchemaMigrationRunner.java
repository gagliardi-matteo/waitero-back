...
public class SchemaMigrationRunner implements ApplicationRunner {
    ...
    private void ensureBillingTables() {
        ...
        jdbcTemplate.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_billing_account_restaurant_unique ON billing_account(ristoratore_id)");
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_billing_account_enabled_day ON billing_account(billing_enabled, billing_day)");
        jdbcTemplate.execute(
                """
                DO $$
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1
                        FROM pg_constraint
                        WHERE conname = 'fk_billing_account_restaurant'
                    ) THEN
        ...
        jdbcTemplate.execute("CREATE UNIQUE INDEX IF NOT EXISTS idx_billing_review_period_unique ON billing_review(ristoratore_id, period_start, period_end)");
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_billing_review_status_created ON billing_review(status, created_at)");
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_billing_review_invoice_id ON billing_review(stripe_invoice_id)");
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_billing_review_restaurant_period_end ON billing_review(ristoratore_id, period_end DESC)");
        jdbcTemplate.execute(
                """
                DO $$
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1
                        FROM pg_constraint
                        WHERE conname = 'fk_billing_review_restaurant'
                    ) THEN
        ...
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_billing_review_order_snapshot_review ON billing_review_order_snapshot(billing_review_id)");
        jdbcTemplate.execute("CREATE INDEX IF NOT EXISTS idx_billing_review_order_snapshot_order ON billing_review_order_snapshot(order_id)");
        jdbcTemplate.execute(
                """
                DO $$
                BEGIN
                    IF NOT EXISTS (
                        SELECT 1
                        FROM pg_constraint
                        WHERE conname = 'fk_billing_review_order_snapshot_review'
                    ) THEN
    ...
    private void ensureBillingGlobalConfigTable() {
        ...
        jdbcTemplate.execute("UPDATE billing_global_config SET updated_at = CURRENT_TIMESTAMP WHERE updated_at IS NULL");
    }
